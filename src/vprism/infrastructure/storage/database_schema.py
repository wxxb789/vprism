...
class DatabaseSchema:
    ...
    def _create_indexes(self) -> None:
        ...
        # 资产信息表索引
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_asset_type ON asset_info(asset_type)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_asset_market ON asset_info(market)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_asset_active ON asset_info(is_active)")
        
        # 日线数据表索引 - 支持按日期和符号的快速查询
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_daily_date ON daily_ohlcv(trade_date)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_daily_symbol_date ON daily_ohlcv(symbol, trade_date DESC)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_daily_market_date ON daily_ohlcv(market, trade_date DESC)")
        
        # 分钟级数据表索引 - 支持时间范围查询
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_intraday_timeframe ON intraday_ohlcv(timeframe)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_intraday_symbol_timeframe ON intraday_ohlcv(symbol, timeframe, timestamp DESC)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_intraday_timestamp ON intraday_ohlcv(timestamp)")
        
        # 实时报价表索引
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_quotes_timestamp ON real_time_quotes(timestamp DESC)")
        
        # 缓存表索引 - 支持过期清理
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_cache_expires ON cache_entries(expires_at)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_cache_data_type ON cache_entries(data_type)")
...
class DatabaseMigration:
    ...
    def migrate_v1_to_v2(self) -> None:
        ...
        # 添加新列
        try:
            conn.execute("ALTER TABLE daily_ohlcv ADD COLUMN adjusted_close DECIMAL(18,6)")
            conn.execute("ALTER TABLE daily_ohlcv ADD COLUMN split_factor DECIMAL(10,6) DEFAULT 1.0")
            conn.execute("ALTER TABLE daily_ohlcv ADD COLUMN dividend_amount DECIMAL(18,6) DEFAULT 0.0")
    ...
    def setup_test_data(self) -> None:
        ...
        # 插入测试资产
        conn.execute("""
            INSERT OR IGNORE INTO asset_info (symbol, market, name, asset_type, currency, exchange)
            VALUES 
        ...
        conn.execute("""
            ...
            VALUES 
                ('000001.SZ', 'cn', '2024-01-01', 10.50, 10.80, 10.30, 10.60, 1000000, 'tushare'),
                ('000001.SZ', 'cn', '2024-01-02', 10.60, 10.90, 10.40, 10.75, 1200000, 'tushare'),
                ('AAPL', 'us', '2024-01-01', 150.00, 152.00, 149.50, 151.50, 50000000, 'yahoo')
        """)
