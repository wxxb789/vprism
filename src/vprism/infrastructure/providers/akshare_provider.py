...
class AkShareProvider(DataProvider):
    ...
    def _discover_capability(self) -> ProviderCapability:
        ...
        return ProviderCapability(
            supported_assets={"stock", "bond", "etf", "fund", "futures", "options", "index"},
            supported_markets={"cn", "us", "hk", "global"},
            supported_timeframes={"tick", "1m", "5m", "15m", "30m", "1h", "4h", "1d", "1w", "1M"},
    ...
    async def get_data(self, query: DataQuery) -> DataResponse:
        ...
        if not self.can_handle_query(query):
            raise ProviderException(f"AkShare cannot handle query: {query}", "akshare")
        ...
        try:
            ...
            raise ProviderException(f"Failed to fetch data from akshare: {e}", "akshare")
    ...
    def _sync_fetch_data(self, query: DataQuery) -> List[DataPoint]:
        ...
        if query.asset == AssetType.STOCK:
            ...
            raise ProviderException(f"Unsupported asset type: {query.asset}", "akshare")
    ...
    def _fetch_us_stock_data(self, symbol: str, query: DataQuery) -> List[DataPoint]:
        ...
        try:
            ...
            # 过滤日期范围
            if query.start:
                stock_us_hist_df = stock_us_hist_df[stock_us_hist_df["date"] >= query.start]
            if query.end:
                stock_us_hist_df = stock_us_hist_df[stock_us_hist_df["date"] <= query.end]
    ...
    def _fetch_hk_stock_data(self, symbol: str, query: DataQuery) -> List[DataPoint]:
        ...
        try:
            ...
            # 过滤日期范围
            if query.start:
                stock_hk_hist_df = stock_hk_hist_df[stock_hk_hist_df["date"] >= query.start]
            if query.end:
                stock_hk_hist_df = stock_hk_hist_df[stock_hk_hist_df["date"] <= query.end]
