...
class VPrismProvider(DataProvider):
    ...
    async def get_data(self, query: DataQuery) -> DataResponse:
        ...
        if not self.can_handle_query(query):
            raise ProviderException(f"VPrism cannot handle query: {query}")
        ...
        try:
            ...
            raise ProviderException(f"Failed to fetch data from vprism: {e}")
    ...
    def _fetch_stock_data(self, symbol: str, query: DataQuery) -> List[DataPoint]:
        ...
        try:
            if query.market == MarketType.CN:
                ...
                if not all(col in stock_zh_a_hist_df.columns for col in required_columns):
                    return data_points
                ...
                # 过滤日期范围
                if query.start:
                    stock_us_daily_df = stock_us_daily_df[stock_us_daily_df["date"] >= query.start]
                if query.end:
                    stock_us_daily_df = stock_us_daily_df[stock_us_daily_df["date"] <= query.end]
            ...
            for _, row in etf_df.iterrows():
                data_point = DataPoint(
                    ...
                    extra_fields={
                        ...
                        "net_asset_value": str(row["累计净值"]) if "累计净值" in row else None
                    }
                )
                data_points.append(data_point)
    ...
    def _validate_price_data(self, df) -> object:
        ...
        # 检查并移除异常值
        numeric_columns = [col for col in df.columns if col in ["开盘", "最高", "最低", "收盘", "open", "high", "low", "close"]]
