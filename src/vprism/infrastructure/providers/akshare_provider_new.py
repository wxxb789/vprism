...
class AkShareProvider(DataProvider):
    ...
    def _discover_capability(self) -> ProviderCapability:
        ...
        return ProviderCapability(
            supported_assets={"stock", "index", "fund", "futures", "options", "bond", "forex", "crypto"},
            supported_markets={"cn", "us", "hk", "global"},
            supported_timeframes={"1min", "5min", "15min", "30min", "60min", "1d", "1w", "1m"},
    ...
    async def get_real_time_quote(self, symbol: str, market: str = "cn") -> Optional[dict]:
        """获取实时报价."""
        if not self._initialized:
            return None
