...
class YFinanceProvider(DataProvider):
    ...
    def _discover_capability(self) -> ProviderCapability:
        ...
        return ProviderCapability(
            supported_assets={"stock", "etf", "index", "crypto", "forex", "futures", "options"},
            supported_markets={"us", "global"},  # Yahoo Finance主要覆盖美股和全球市场
            supported_timeframes={"1m", "2m", "5m", "15m", "30m", "1h", "1d", "5d", "1wk", "1mo", "3mo"},
    ...
    async def get_data(self, query: DataQuery) -> DataResponse:
        ...
        if not self.can_handle_query(query):
            raise ProviderException(f"YFinance cannot handle query: {query}", "yfinance")
        ...
        try:
            ...
            raise ProviderException(f"Failed to fetch data from yfinance: {e}", "yfinance")
