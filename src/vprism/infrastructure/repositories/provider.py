...
from vprism.infrastructure.storage import DatabaseManager, ProviderRecord
...
class ProviderRepository(Repository[ProviderRecord]):
    ...
    async def find_by_id(self, entity_id: str) -> Optional[ProviderRecord]:
        """根据ID查找提供商记录."""
    ...
    async def find_by_name(self, name: str) -> Optional[ProviderRecord]:
        """根据名称查找提供商记录."""
    ...
    async def find_all(self, limit: Optional[int] = None, offset: int = 0) -> List[ProviderRecord]:
        """查找所有提供商记录."""
        ...
        if record:
            if record.avg_response_time_ms is None:
                ...
                record.avg_response_time_ms = (
                    (record.avg_response_time_ms * (record.request_count - 1) + response_time_ms) 
