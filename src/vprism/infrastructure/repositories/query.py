...
class QueryRepository(Repository[QueryRecord]):
    ...
    async def find_by_id(self, entity_id: str) -> Optional[QueryRecord]:
        """根据ID查找查询记录."""
        # 简化的实现
        return None
    
    async def find_all(self, limit: Optional[int] = None, offset: int = 0) -> List[QueryRecord]:
        """查找所有查询记录."""
        # 简化的实现
        return []
    ...
    async def find_recent_queries(self, 
    ...
    async def create_query_record(self, 
                                query: DataQuery,
