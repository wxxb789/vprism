...
from vprism.infrastructure.storage import DatabaseManager, CacheRecord
...
class CacheRepository(Repository[CacheRecord]):
    ...
    async def find_by_id(self, entity_id: str) -> Optional[CacheRecord]:
        """根据ID查找缓存记录."""
    ...
    async def find_by_cache_key(self, cache_key: str) -> Optional[CacheRecord]:
        """根据缓存键查找缓存记录."""
        return await self.find_by_id(cache_key)
    
    async def find_all(self, limit: Optional[int] = None, offset: int = 0) -> List[CacheRecord]:
        """查找所有缓存记录."""
