...
from vprism.core.models import DataQuery, DataPoint
from vprism.infrastructure.storage import DatabaseManager, DataRecord
...
class DataRepository(Repository[DataRecord]):
    ...
    async def find_by_id(self, entity_id: str) -> Optional[DataRecord]:
        """根据ID查找数据记录."""
    ...
    async def find_all(self, limit: Optional[int] = None, offset: int = 0) -> List[DataRecord]:
        """查找所有数据记录."""
    ...
    async def find_by_symbol(self, symbol: str, limit: Optional[int] = None) -> List[DataRecord]:
        """根据股票代码查找数据记录."""
    ...
    async def find_by_date_range(self, 
                                start_date: datetime, 
                                end_date: datetime,
                                symbol: Optional[str] = None) -> List[DataRecord]:
        """根据日期范围查找数据记录."""
        records = self.db_manager.query_data_records(
            symbol=symbol,
    ...
    async def get_earliest_data(self, symbol: str) -> Optional[DataRecord]:
        """获取最早数据记录."""
