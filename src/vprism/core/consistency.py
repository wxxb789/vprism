...
class DataConsistencyValidator:
    ...
    def validate_consistency(self, 
                           ...
                           start_date: datetime,
                           end_date: datetime,
                           timeframe: str = "1d") -> ConsistencyReport:
        """Validate data consistency for a specific symbol and date range."""
        self.logger.info(f"Validating consistency for {symbol} from {start_date} to {end_date}")
        ...
        # Perform comparison
        return self._compare_data(symbol, vprism_data, akshare_data, start_date, end_date)
    
    def _get_vprism_data(self, 
                        symbol: str, 
                        start_date: datetime, 
                        end_date: datetime,
    ...
    def _get_akshare_data(self, 
        ...
        try:
            ...
            from vprism.core.models import Asset, AssetType, MarketType, DataQuery, TimeFrame
    ...
    def _compare_data(self, 
                     ...
                     start_date: datetime,
                     end_date: datetime) -> ConsistencyReport:
        ...
        if 'timestamp' not in vprism_df.columns or 'timestamp' not in akshare_df.columns:
            return ConsistencyReport(
                symbol=symbol,
                start_date=start_date,
                end_date=end_date,
        ...
        common_timestamps = set(vprism_df['timestamp']).intersection(set(akshare_df['timestamp']))
        ...
        missing_in_vprism = len(set(akshare_df['timestamp']) - set(vprism_df['timestamp']))
        missing_in_akshare = len(set(vprism_df['timestamp']) - set(akshare_df['timestamp']))
            ...
            for field in price_fields:
                ...
                if vprism_val is not None and akshare_val is not None:
                    ...
                    if relative_diff > self.tolerance:
                        ...
                        issues.append(f"{timestamp}: {field} differs by {relative_diff:.2%}")
                    else:
                        matching_records += 1
        ...
        consistency_percentage = (matching_records / max(total_common * 4, 1)) * 100 if total_common > 0 else 0.0
    ...
    def compare_multiple_symbols(self, 
                                 ...
                                 start_date: datetime,
                                 end_date: datetime,
                                 timeframe: str = "1d") -> Dict[str, ConsistencyReport]:
        """Compare consistency for multiple symbols."""
        reports = {}
        
        for symbol in symbols:
            try:
                report = self.validate_consistency(symbol, start_date, end_date, timeframe)
    ...
    def generate_consistency_report(self, reports: Dict[str, ConsistencyReport]) -> str:
        ...
        for symbol, report in reports.items():
            lines.append(f"\nSymbol: {symbol}")
            lines.append("-" * 40)
            lines.append(f"Date Range: {report.start_date.strftime('%Y-%m-%d')} to {report.end_date.strftime('%Y-%m-%d')}")
            ...
            lines.append(f"Average Price Difference: {report.average_price_difference:.2%}")
            lines.append(f"Max Price Difference: {report.max_price_difference:.2%}")
            lines.append(f"Consistency Percentage: {report.consistency_percentage:.1f}%")
        ...
        if overall_consistency:
            ...
            lines.append("-" * 20)
            lines.append(f"Average Consistency Across All Symbols: {avg_consistency:.1f}%")
            lines.append(f"Symbols with >95% consistency: {sum(1 for c in overall_consistency if c > 95)}")
            lines.append(f"Symbols with <90% consistency: {sum(1 for c in overall_consistency if c < 90)}")
    ...
    def run_automated_validation(self, 
                                ...
                                days_back: int = 30,
                                timeframe: str = "1d",
                                alert_threshold: float = 95.0) -> Dict[str, Any]:
        """Run automated consistency validation with alerting."""
        end_date = datetime.now()
        start_date = end_date - timedelta(days=days_back)
        
        reports = self.compare_multiple_symbols(symbols, start_date, end_date, timeframe)
        
        # Generate summary
        alert_symbols = []
        summary = {
            'total_symbols': len(symbols),
            'validated_symbols': len([r for r in reports.values() if r.total_records > 0]),
            'failed_symbols': len([r for r in reports.values() if r.total_records == 0]),
        ...
        for symbol, report in reports.items():
            if report.consistency_percentage < alert_threshold and report.total_records > 0:
