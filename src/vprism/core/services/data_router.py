...
from vprism.core.interfaces.providers import DataProvider, ProviderCapability
...
class DataRouter:
    ...
    async def route_query(self, query: DataQuery) -> DataProvider:
        ...
        Raises:
            NoCapableProviderException: 当没有可行的提供商时
        """
        # 查找所有可行的提供商
        ...
        if not capable_providers:
            ...
            raise NoCapableProviderException(
        ...
        logger.info(f"Routing query to best provider: {best_provider.name} (score: {self.provider_scores[best_provider.name]:.2f})")
    ...
    def _select_best_provider(self, providers: List[DataProvider], query: DataQuery) -> DataProvider:
        ...
        logger.debug(
            f"Provider selection scores: {[f'{p.name}:{s:.2f}' for s, p in scored_providers]}"
        )
