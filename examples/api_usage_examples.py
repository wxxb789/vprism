...
def simple_api_examples():
    ...
    try:
        ...
        print(
            "   vprism.get(asset=AssetType.STOCK, market=MarketType.CN, symbols=['000001'])"
        )
...
def builder_api_examples():
    ...
    try:
        ...
        base_query = vprism.query().asset(AssetType.STOCK).market(MarketType.CN)

        # Create different variations
        daily_query = (
            base_query.copy().timeframe(TimeFrame.DAY_1).symbols(["000001"]).build()
        )
        hourly_query = (
            base_query.copy().timeframe(TimeFrame.HOUR_1).symbols(["000002"]).build()
        )
        ...
        multi_symbol_query = (
            builder.copy().symbol("000001").symbol("000002").symbol("000003").build()
        )
...
async def async_api_examples():
    ...
    try:
        ...
        query = vprism.query().asset(AssetType.STOCK).symbols(["000001"]).build()
        print(f"   Query: {query}")
