...
class AuthConfig:
    ...
    def is_valid(self) -> bool:
        ...
        if self.auth_type == AuthType.NONE:
            ...
            return (
                "username" in self.credentials and 
                "password" in self.credentials and
                bool(self.credentials["username"]) and
                bool(self.credentials["password"])
            )
...
class EnhancedProviderRegistry:
    ...
    def find_capable_providers(self, query: DataQuery) -> List[EnhancedDataProvider]:
        ...
        capable_providers.sort(
            key=lambda p: self._provider_scores.get(p.name, 1.0),
            reverse=True
        )
...
class MockDataProvider(EnhancedDataProvider):
    ...
    def __init__(
        ...
        self._name = name
        self._supported_assets = supported_assets if supported_assets is not None else {AssetType.STOCK}
