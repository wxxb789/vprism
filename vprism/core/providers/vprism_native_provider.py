...
class AkshareModernAdapter:
    ...
    def standardize_dataframe(
        ...
        column_mapping = self._column_mappings.get(asset_key, self._column_mappings["stock"])
        
        # Normalize column names
        df_normalized = df.copy()
        df_normalized.columns = [
            column_mapping.get(col, col) for col in df.columns
        ]
    ...
    async def _build_function_params(
        ...
        if query.start:
            ...
            if any(keyword in str(params) for keyword in ["hist", "daily", "weekly", "monthly"]):
    ...
    async def get_data(self, query: DataQuery) -> DataResponse:
        ...
        try:
            ...
            metadata = ResponseMetadata(
                ...
                cache_hit=False,
                warnings=[] if all_data_points else ["No data returned from vprism native"]
            )
