...
class TestAkshareModernAdapter:
    ...
    def test_standardize_dataframe_etf_data(self, adapter):
        ...
        assert "累计净值" in point.extra_fields or "cumulative_nav" in point.extra_fields
    ...
    async def test_fetch_data_success(self, adapter):
        ...
        with patch('vprism.core.providers.vprism_native_provider.AKSHARE_AVAILABLE', True):
            with patch('vprism.core.providers.vprism_native_provider.ak', create=True) as mock_ak:
...
class TestVPrismNativeProvider:
    ...
    def test_initialization_without_akshare(self):
        ...
        with patch('vprism.core.providers.vprism_native_provider.AKSHARE_AVAILABLE', False):
            with pytest.raises(ProviderException) as exc_info:
    ...
    def test_can_handle_query_unsupported(self, vprism_native_provider):
        ...
        query = DataQuery(
            asset=AssetType.STOCK,
            market=MarketType.CN,
            symbols=[f"00000{i}" for i in range(20)]  # More than max_symbols_per_request
        )
    ...
    @pytest.mark.asyncio
    async def test_get_data_multiple_symbols(self, vprism_native_provider, mock_akshare):
    ...
    @pytest.mark.asyncio
    async def test_get_data_no_symbols_provided(self, vprism_native_provider, mock_akshare):
    ...
    @pytest.mark.asyncio
    async def test_get_default_symbols_stock_cn(self, vprism_native_provider, mock_akshare):
    ...
    @pytest.mark.asyncio
    async def test_get_default_symbols_etf_cn(self, vprism_native_provider, mock_akshare):
    ...
    @pytest.mark.asyncio
    async def test_get_default_symbols_error_handling(self, vprism_native_provider, mock_akshare):
    ...
    async def test_adapter_integration(self, vprism_native_provider, mock_akshare):
        ...
        with patch.object(vprism_native_provider._adapter, 'fetch_data') as mock_fetch:
            ...
            with patch.object(vprism_native_provider._adapter, 'standardize_dataframe') as mock_standardize:
...
class TestVPrismNativeProviderIntegration:
            ...
            with patch('vprism.core.providers.akshare_provider.AKSHARE_AVAILABLE', True):
                with patch('vprism.core.providers.vprism_native_provider.ak', create=True):
                    with patch('vprism.core.providers.akshare_provider.ak', create=True):
                        ...
                        registry.update_provider_score("vprism_native", True, 100)  # Good performance
                        registry.update_provider_score("akshare", True, 500)        # Slower performance
    ...
    def test_function_mapping_completeness(self):
        ...
        expected_keys = [
            "stock_cn_spot", "stock_cn_daily", "stock_cn_intraday",
            "stock_hk_spot", "stock_hk_daily",
            "stock_us_spot", "stock_us_daily",
            "etf_cn_spot", "etf_cn_daily",
            "fund_cn_open", "fund_cn_money",
            "bond_cn_spot", "bond_cn_treasury",
            "futures_cn_spot", "futures_cn_daily",
            "index_cn_spot", "index_cn_daily",
            "crypto_spot"
        ]
