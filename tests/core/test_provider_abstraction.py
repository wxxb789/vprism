...
class TestAuthConfig:
    ...
    def test_auth_config_creation_api_key(self):
        ...
        config = AuthConfig(
            auth_type=AuthType.API_KEY, credentials={"api_key": "test_key_123"}
        )
    ...
    def test_auth_config_creation_bearer_token(self):
        ...
        config = AuthConfig(
            auth_type=AuthType.BEARER_TOKEN, credentials={"token": "bearer_token_123"}
        )
    ...
    def test_auth_config_creation_none(self):
        ...
        config = AuthConfig(auth_type=AuthType.NONE, credentials={})
    ...
    def test_auth_config_validation_api_key_missing(self):
        ...
        config = AuthConfig(auth_type=AuthType.API_KEY, credentials={})
...
class TestRateLimitConfig:
    ...
    def test_rate_limit_config_defaults(self):
        ...
        config = RateLimitConfig(requests_per_minute=60, requests_per_hour=3600)
    ...
    def test_rate_limit_config_validation_positive_values(self):
        ...
        with pytest.raises(ValueError):
            RateLimitConfig(requests_per_minute=-1, requests_per_hour=100)
...
class TestMockProviders:
    ...
    async def test_mock_provider_data_generation(self):
        ...
        mock_provider = create_mock_provider(
            name="test_mock", supported_assets={AssetType.STOCK}
        )
...
class TestProviderAbstractionEdgeCases:
    ...
    def test_rate_limit_config_edge_cases(self):
        ...
        config = RateLimitConfig(
            requests_per_minute=10000, requests_per_hour=100000, concurrent_requests=100
        )
        ...
        config = RateLimitConfig(
            requests_per_minute=1, requests_per_hour=1, concurrent_requests=1
        )
    ...
    def test_provider_capability_history_limits(self):
        ...
        recent_query = DataQuery(
            asset=AssetType.STOCK, start=datetime.now() - timedelta(days=20)
        )
        assert capability.matches_query(recent_query)

        # Query beyond history limit
        old_query = DataQuery(
            asset=AssetType.STOCK, start=datetime.now() - timedelta(days=60)
        )
    ...
    def test_provider_capability_real_time_requirements(self):
        ...
        historical_query = DataQuery(
            asset=AssetType.STOCK, start=datetime.now() - timedelta(days=1)
        )
    ...
    async def test_mock_provider_streaming_without_support(self):
        ...
        mock_provider = create_mock_provider(
            name="no_streaming", supports_streaming=False
        )
...
class TestProviderAbstractionIntegration:
    ...
    async def test_complete_provider_lifecycle_with_capabilities(self):
        ...
        class IntegrationTestProvider(EnhancedDataProvider):
            def __init__(self):
                auth_config = AuthConfig(
                    AuthType.API_KEY, {"api_key": "integration_test_key"}
                )
        ...
        capable_providers_after_health_check = registry.find_capable_providers(
            stock_query
        )
    ...
    def test_rate_limiting_integration(self):
        ...
        config = RateLimitConfig(
            requests_per_minute=60, requests_per_hour=1000, concurrent_requests=3
        )
