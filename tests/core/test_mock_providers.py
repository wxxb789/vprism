...
class TestMockDataProvider:
    ...
    async def test_health_check_with_failure_rate(self):
        ...
        assert not all(health_results), (
            "Expected some health checks to fail due to failure rate"
        )
...
class TestSpecializedProvider:
    ...
    def test_specialized_market_support(self):
        ...
        provider = SpecializedProvider("cn_specialist", AssetType.STOCK, MarketType.CN)
...
class TestProviderSuite:
    ...
    def test_provider_suite_diversity(self):
        ...
        stock_providers = [
            p for p in suite.values() if AssetType.STOCK in p.supported_assets
        ]
        crypto_providers = [
            p for p in suite.values() if AssetType.CRYPTO in p.supported_assets
        ]
        bond_providers = [
            p for p in suite.values() if AssetType.BOND in p.supported_assets
        ]
