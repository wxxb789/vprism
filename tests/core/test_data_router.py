    ...
    async def get_data(self, query: DataQuery) -> DataResponse:
        ...
        if self._should_fail:
            raise ProviderException(
                f"Mock failure from provider {self.name}", provider=self.name
            )
...
class TestDataRouter:
    ...
    def test_data_router_route_query_basic(self, populated_registry, mock_providers):
        ...
        assert selected_provider.name in [
            "fast_provider",
            "slow_provider",
            "multi_provider",
        ]
    ...
    def test_data_router_route_query_prefers_low_latency(
        self, populated_registry, mock_providers
    ):
    ...
    def test_data_router_route_query_considers_health(
        self, populated_registry, mock_providers
    ):
    ...
    def test_data_router_update_provider_score_high_latency_penalty(
    ...
    def test_data_router_score_bounds(self, populated_registry):
        ...
        for _ in range(20):
            router.update_provider_score(
                "fast_provider", success=False, latency_ms=10000
            )
    ...
    def test_data_router_route_with_scoring(self, populated_registry, mock_providers):
        ...
        test_registry = EnhancedProviderRegistry()
        high_score_provider = MockProvider(
            "high_score", {AssetType.STOCK}, data_delay_seconds=0
        )
        low_score_provider = MockProvider(
            "low_score", {AssetType.STOCK}, data_delay_seconds=0
        )
        ...
        assert high_score_count > low_score_count, (
            f"High score: {high_score_count}, Low score: {low_score_count}"
        )

    @pytest.mark.asyncio
    async def test_data_router_execute_query_success(
        self, populated_registry, mock_providers
    ):
    ...
    @pytest.mark.asyncio
    async def test_data_router_execute_query_with_fallback(
        self, populated_registry, mock_providers
    ):
    ...
    @pytest.mark.asyncio
    async def test_data_router_execute_query_all_providers_fail(
        self, populated_registry, mock_providers
    ):
    ...
    @pytest.mark.asyncio
    async def test_data_router_execute_query_updates_scores(
        self, populated_registry, mock_providers
    ):
    ...
    @pytest.mark.asyncio
    async def test_data_router_health_check_integration(
        self, populated_registry, mock_providers
    ):
    ...
    def test_data_router_concurrent_access(self, populated_registry):
        ...
        def update_scores():
            try:
                for i in range(50):
                    router.update_provider_score(
                        "fast_provider", success=True, latency_ms=100 + i
                    )
                    time.sleep(
                        0.001
    ...
    @pytest.mark.asyncio
    async def test_data_router_circuit_breaker_behavior(
        self, populated_registry, mock_providers
    ):
        ...
        assert failing_score < 2.0, (
            f"Failing provider score should be reduced from 2.0, got {failing_score}"
        )
