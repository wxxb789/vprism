...
from vprism.infrastructure.cache import CacheKey, ThreadSafeInMemoryCache, SimpleDuckDBCache, MultiLevelCache
from vprism.infrastructure.cache.base import CacheStrategy
    ...
    async def test_concurrent_cache_operations(self, cache):
        ...
        async def worker(worker_id):
            for i in range(5):
                query = DataQuery(
                    asset=AssetType.STOCK,
                    symbols=[f"worker{worker_id}_stock{i}"]
                )
